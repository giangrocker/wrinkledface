...
inline cv::Mat estimate_affine_camera(vector<Vector2f> image_points,
                                      vector<Vector4f> model_points)
{
    ...
    {
        Mat imgPoint(1, 2, CV_32FC1);
        imgPoint.at<float>(0, 0) = image_points.at(i)(0);
        imgPoint.at<float>(0, 1) = image_points.at(i)(1);
        matImagePoints.push_back(imgPoint);

        Mat mdlPoint(1, 3, CV_32FC1);
        mdlPoint.at<float>(0, 0) = model_points.at(i)(0);
        mdlPoint.at<float>(0, 1) = model_points.at(i)(1);
        mdlPoint.at<float>(0, 2) = model_points.at(i)(2);
        matModelPoints.push_back(mdlPoint);
    }
...
*/
inline cv::Mat project_affine(cv::Vec4f vertex, cv::Mat affine_camera_matrix)
{
    // Transform to clip space:
    cv::Mat clip_coords = affine_camera_matrix * cv::Mat(vertex);
